...
def add_song(request):
    ...
    if request.method == "POST":
        ...
        # Reference cis now a bound instance with user data sent in POST
        # Call is_valid() to validate data and create cleaned_data and errors dict
        if form.is_valid():
            action='POST_SUCCESFULL'
            #form data is valid, you can access vvlaidate values in the cleaned_data dict
            # e.g form.cleaned_data['data']
        else:
            action='POST_FAILED'
    else:
        action='GET'
